// app/api/did-agent/sdp/route.ts
// Sends the browser's SDP answer to D-ID to complete the WebRTC handshake for an Agent stream.
...
export async function POST(req: Request) {
  try {
    ...
    if (!res.ok) {
      return Response.json({ error: "Agent SDP exchange failed", details: data }, { status: res.status });
    }
